    ...
    <div>
        {cart.length === 0 ? (
          ...
          cart.map((data) => {
            ...
            return (
              <div  className='flex flex-col hover:shadow-gray-400 hover:bg-gray-100 rounded-xl hover:shadow-lg  py-5 gap-3  justify-between w-full items-center '>
              <Link onClick={() => dispatch(openCart())} to={`/product/${data.title}/${data._id}`}  className='flex  rounded-xl  flex-row py-5 gap-3  justify-between w-full items-center ' key={data._id}>
                <div className=' w-[150px] h-[150px] ml-[6%] ' >
              ...
              <button className=' z-100 underline texxt-[13px] ' onClick={() => dispatch(removeFromCart(data))}>Remove</button>
